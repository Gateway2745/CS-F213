...
class Dealer implements Runnable {
	...
	public void run() {
		...
		synchronized(gameData.lock1){
			// dealer executes until either (or both) players declare success 
				...
				synchronized(gameData.lock2) {
					...
					while(numberAnnounced == 0) {				
						try {
							System.out.println("dealer waiting for gui number");
							gameData.lock2.wait();	
							System.out.println("dealer has woken");
				...
				while(!gameData.playerChanceFlag[0] || !gameData.playerChanceFlag[1]) {
					try {
						gameData.lock1.wait();
						System.out.println("dealer waiting for players to finish marking....");
					} catch (InterruptedException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					}
				}
				System.out.println("dealer ready to deal again as players have marked....");
			}
			...
			System.out.println("dealing over !!");
		}		
	}
...
class Player implements Runnable {
	...
	public void run() {
		...
		synchronized(gameData.lock1){			
			...
			System.out.println("Player " + id + " is executing");
			while(!gameData.playerSuccessFlag[0] && !gameData.playerSuccessFlag[1]) {
				...
				while(!gameData.noAnnouncedFlag || gameData.playerChanceFlag[id])
				{
					try {
						gameData.lock1.wait();
						System.out.println("player " + id + " waiting as already played or number not yet announced");
					} catch (InterruptedException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					}
				...
				if(!gameData.gameCompleteFlag) {					
					...
					System.out.println("player " + id + " has finished marking");
		...
		for(int i = 0; i < 30; i++) {			
			if(e.getSource() == btnDealerBoardNumbers[i]) {				
				...
				synchronized(gameData.lock2) {									
					...
					System.out.println("number set by gui and dealer can continue..");
				}				
				break;
			}
		}
